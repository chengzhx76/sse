...
func TestClientChanSubscribeEmptyMessage(t *testing.T) {
	...
	events := make(chan *Event)
	err := c.SubscribeChan("test", "test", events)
	require.Nil(t, err)
...
func TestClientOnDisconnect(t *testing.T) {
	...
	go c.Subscribe("test", "test", func(msg *Event) {})
...
func TestClientUnsubscribe401(t *testing.T) {
	...
	err := c.SubscribeRaw("test", func(ev *Event) {
		// this shouldn't run
		assert.False(t, true)
	})
...
func TestClientLargeData(t *testing.T) {
	...
	go func() {
		c.Subscribe("test", "test", func(ev *Event) {
			ec <- ev
		})
	}()
...
func TestSubscribeWithContextDone(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		go c.SubscribeWithContext(ctx, "test", "test", func(msg *Event) {})
	}
